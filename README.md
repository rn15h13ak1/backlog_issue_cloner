...
- 複製先のプロジェクトはその課題から決まるため、`target_project_key` とは同時に指定できません。
- コピー元と同じ課題を指定した場合はエラーになります。

## プロジェクトを跨いだ複製

**コピー元とは別のプロジェクトに複製できます。** 指定方法は 2 通りです。

| 指定 | 複製先 | 使えるモード |
|---|---|---|
| `clone.target_project_key` | そのプロジェクト | 単純複製 / 定期作成 |
| `clone.target_issue_key` | その課題があるプロジェクト | 直接更新 |

```yaml
clone:
  source_issue_key: "PROJ-10"     # コピー元は PROJ プロジェクト
  target_project_key: "DEST"      # 複製先は DEST プロジェクト
```
...
作成完了: DEST-101 — 月次点検手順
  [子] 作成完了: DEST-102 — 手順1 バックアップ
  [子] 作成完了: DEST-103 — 手順2 検証
  [子] 作成完了: DEST-104 — 手順3 報告
```

重複判定も複製先のプロジェクト内で行われます。子課題も同じ複製先に作られます。

### 属性の引き継がれ方

| 項目 | 挙動 |
|---|---|
| 種別 | **名前で照合し直します。** 複製先に同名の種別があればその ID を使い、無ければ複製先の既定の種別にフォールバックします |
| 優先度 | Backlog スペース共通のため、そのまま引き継がれます |
| 担当者 / カテゴリー / マイルストーン | もともとコピーされません。これらはプロジェクト固有なので、跨いだ場合は特に引き継げません |

種別 ID はプロジェクトごとに異なるため、ID をそのまま渡すと失敗します。そのため名前で解決し直しています。
コピー元の子課題が「バグ」種別でも、複製先に「バグ」が無ければ親と同じ種別で作られます。

### 対話メニューからの制限

**`menu.py` は `target_project_key` を尋ねません。** メニューからプロジェクトを跨ぐ場合は、
**直接更新モード**で複製先プロジェクトの課題キーを指定してください。

単純複製・定期作成でプロジェクトを跨ぐ場合は、`config.yaml` に `target_project_key` を書くか、
本体を直接呼んでください。

```bash
python3 backlog_issue_cloner.py --source-issue-key PROJ-10 \
    --config cross-project.yaml --execute
```
...
複製先の課題キーを聞かれます。ここでは複製先 `DEST-5` が次の状態だとします。

```
DEST-5  9月 月次点検          ← 本文がコピー元と異なる
├── DEST-6  手順1 バックアップ  ← 本文は同じ
└── DEST-7  手順2 検証         ← 本文が古い
                              （「手順3 報告」は無い）
```
...
設定値:
  件名        : 9月 月次点検
  対象PJ      : DEST (id=20)
  コピー先    : DEST-5（件名は変更しません）
  子課題照合  : substring
既存課題あり: DEST-5

実行内容:
  [親] 本文を更新  9月 月次点検（DEST-5）
  [子] 変更なし    手順1 バックアップ（DEST-6）
  [子] 本文を更新  手順2 検証（DEST-7）
  [子] 新規作成    手順3 報告
  Backlog に反映しますか？（新規作成 1 件 / 本文更新 2 件） [y/N]: y
更新完了: DEST-5 — 9月 月次点検
  [子] 更新完了: DEST-7 — 手順2 検証
  [子] 作成完了: DEST-101 — 手順3 報告
```
...
```
実行内容:
  [親] 変更なし     【定期】20260828 タスク（PROJ-99）
  [子] 変更なし     手順1 バックアップ（DEST-6）
  [子] 新規作成     手順2 検証
...
| 種別 | コピー元の子課題と**同名の種別**を複製先プロジェクトで探して使う。無ければ親課題と同じ種別（[プロジェクトを跨いだ複製](#プロジェクトを跨いだ複製)を参照） |
...
| `target_project_key` | | コピー元と同じ | 作成先プロジェクトキー（[プロジェクトを跨いだ複製](#プロジェクトを跨いだ複製)を参照） |
| `target_issue_key` | | — | 複製先の課題キー。指定すると件名で検索せずこの課題を直接更新する（[複製先を直接指定する](#複製先を直接指定する)を参照） |
| `issue_type` | | 最初の種別 | 種別名。見つからない場合は警告して最初の種別を使う |
| `priority` | | `中` | 優先度名（`最高` / `高` / `中` / `低`）。見つからない場合は警告して `中`、それも無ければ最初の優先度を使う |
...
- **コピーされるのは本文（description）だけです。** 担当者・カテゴリ・マイルストーン・期限日・カスタム属性は引き継がれません（子課題の種別・優先度・親子関係は引き継がれます）。
