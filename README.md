...
同じ件名の未完了課題が既にある場合は作成せず、本文に差分があれば更新します。
そのため cron などで繰り返し実行しても課題が重複しません。

ただし既定では**完了済みの課題は検索対象外**です。作成した課題を完了にした後で同じ日付の実行を繰り返すと、
同じ件名の課題がもう 1 件作られます。これを避けたい場合は `include_closed: true` を設定してください
（[重複判定の挙動](#重複判定の挙動)を参照）。

## 動作の流れ

```
コピー元課題 (PROJ-123) の本文と子課題を取得
        ↓
件名テンプレートに日付を埋め込む  →  「【定期】20260828 タスク」
        ↓
同じ件名の未完了課題を検索（target_issue_key 指定時はその課題を直接使う）
        ↓
   ┌─────────────────┬──────────────────┬─────────────────┐
  ...
  新規作成          何もしない          本文を更新
        ↓
子課題ごとに、複製先の親に紐づく子課題と突き合わせて
新規作成 / 本文を更新 / 何もしない を決める
        ↓
実行内容を一覧表示 → まとめて 1 回だけ確認 → 反映
```
...
| `summary_template` | | `{SOURCE_SUMMARY}` | 件名テンプレート。省略するとコピー元の件名をそのまま使う |
...
| 種別 | コピー元の子課題と**同名の種別**を複製先プロジェクトで探して使う。無ければ親課題と同じ種別 |
...
`summary_template` は省略できます。省略すると**コピー元の件名をそのまま**使います。

| テンプレート | コピー元が「月次点検」の場合 |
|---|---|
| 省略（デフォルト） | `月次点検` |
| `【定期】{YYYYMMDD} タスク` | `【定期】20260909 タスク` |
| `{YYYYMMDD} {SOURCE_SUMMARY}` | `20260909 月次点検` |

使えるプレースホルダは `{SOURCE_SUMMARY}`（コピー元の件名）と `{YYYYMMDD}`（`--date` または今日の日付）です。

> **省略した場合の注意**
> 生成される件名がコピー元と同じになるため、コピー元自身は重複判定から自動的に除外されます。
> ただし日付が入らないので、**同じ件名の課題が既にあれば新規作成ではなく更新**になります。
> 実行のたびに別の課題を立てたい場合は `{YYYYMMDD}` を含めてください。
...
`target_issue_key` を指定しない場合、複製先は件名で検索して決まります。
