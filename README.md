...
**Windows では `menu.bat` をダブルクリック**しても起動できます。
Python 3.10 以上と PyYAML が入っていれば、コマンドを打たずに使えます。
