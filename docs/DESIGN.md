...
### Windows の日本語コンソールは CP932

`menu.bat` からの起動を含め、Windows の既定のコンソールは CP932 です。
**CP932 に無い文字を `print` すると、文字化けではなく `UnicodeEncodeError` で落ちます。**

以前は `―` の代わりに em dash（U+2014）を使っており、メニューの最初の画面すら
表示できませんでした。`tests/test_windows.py` が出力ソースの全文字を検査しています。

`menu.bat` は ASCII のみ・CRLF で書き、`cd` ではなく `pushd "%~dp0"` を使います。
`cd` はネットワーク共有（UNC パス）で失敗するためです。
改行コードは `.gitattributes` の `*.bat text eol=crlf` で固定しています。
...
`scripts/check_docs.py` が次の 2 つを検査します。

- **`**` による強調の両端は半角スペース。** 句読点や括弧に隣接する場合も入れます。
  日本語文字に直接隣接すると、環境によって強調が描画されないためです。
- ` ``` ` の直後が `====` などの罫線だけの行になる場合は、 **間に空行を入れる。**
  入れないとプレビュー表示が崩れます。

リンクとアンカーは、共通規約の `../ws-conventions/bin/check-markdown.sh` が検査します
（リポジトリの外を指すものも含む）。`check_docs.py` でも見ていましたが、コードフェンスを
剥がさずに走査しており、 **コード例の中に書いたリンクを実在しないものとして報告して
いました。** 二重に持つ理由も無いため外しています。
